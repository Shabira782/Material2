...
class TotalPemesananModel extends Model
{
    ...
    public function getDataPemesanan($area, $jenis, $tgl_pakai)
    {
        $query = $this->db->table('total_pemesanan tp')
            ->select("tp.id_total_pemesanan, 
            pp.id_pengeluaran, 
            tp.ttl_jl_mc, tp.ttl_kg, 
            tp.ttl_cns, p.id_pemesanan, 
            p.tgl_pakai, m.area, 
            ...
            SUM(CASE WHEN pp.status = 'Pengiriman Area' THEN pp.kgs_out ELSE 0 END) AS kg_kirim, 
            SUM(CASE WHEN pp.status = 'Pengiriman Area' THEN pp.cns_out ELSE 0 END) AS cns_kirim, 
            SUM(CASE WHEN pp.status = 'Pengiriman Area' THEN pp.krg_out ELSE 0 END) AS krg_kirim, 
            GROUP_CONCAT(DISTINCT CASE WHEN pp.status = 'Pengiriman Area' THEN pp.lot_out ELSE NULL END) AS lot_kirim, 
            GROUP_CONCAT(DISTINCT CASE WHEN pp.status = 'Pengiriman Area' THEN pp.nama_cluster ELSE NULL END) AS cluster_kirim, 
